...
let CheckOutService = class CheckOutService extends base_service_1.BaseService {
    getCheckOutPosition() {
        return this.findById('001', 'check-out-position');
    }
    check(lat1, lng1, lat2, lng2) {
    ...
    checkInOrOut(lat, lng) {
        ...
        if (r >= r1) {
            return ResultData_1.ResultData.end(200, Object.assign({}, checkOut), "在范围内");
        }
        else {
            return ResultData_1.ResultData.end(500, Object.assign({}, checkOut), "不在范围内");
        }
    }
    checkIn(lat, lng, token) {
        ...
        let r = checkOut.r;
        let r1 = this.check(lat, lng, lat1, lng1);
        let obj = Object.assign(Object.assign(Object.assign({}, checkOut), userInfo), { id: new Date().getTime(), userId: userInfo.id, lat: lat, lng: lng, lat1: lat1, lng1: lng1 });
        let d = new Date();
        ...
        return ResultData_1.ResultData.end(200, obj, '打卡成功');
    }
    getRecordSimple(token) {
        let userInfo = this.getAdminUserByToken(token);
        let d = new Date();
        let year = d.getFullYear();
        let month = d.getMonth() + 1;
        ...
        console.log(endInfo);
        return ResultData_1.ResultData.end(200, { beginTime: beginNum, endTime: endNum }, '查询成功');
    }
    getRecordFull(date, token) {
        let userInfo = this.getAdminUserByToken(token);
        let list = this.findAll('check-out-record').getData.list;
        list = list.filter(item => {
